...
static struct image_convert_ctx *
ipu_image_convert_next(struct ipu_ic_task *task)
{
	struct ipu_ic_priv *priv = task->priv;
	struct image_convert_ctx *ctx;
	...
	ctx = list_first_entry(&task->image_list, struct image_convert_ctx,
			       list);
	...
	ipu_cpmem_zero(task->input_channel);
	ipu_cpmem_zero(task->output_channel);
	...
	ipu_cpmem_set_image(task->input_channel, &ctx->in);
	ipu_cpmem_set_image(task->output_channel, &ctx->out);

	ipu_cpmem_set_burstsize(task->input_channel, inburst);
	ipu_cpmem_set_burstsize(task->output_channel, outburst);

	if (ctx->deinterlace) {
		ipu_cpmem_zero(task->input_channel_p);
		ipu_cpmem_zero(task->input_channel_n);
		...
		ipu_cpmem_set_image(task->input_channel_p, &ctx->in_p);
		ipu_cpmem_set_image(task->input_channel_n, &ctx->in_n);

		ipu_cpmem_set_burstsize(task->input_channel_p, inburst);
		ipu_cpmem_set_burstsize(task->input_channel_n, inburst);
	}
		      ...
		      void *complete_context)
{
	...
	dev_dbg(priv->dev,
		"%s: in: %dx%d(%dx%d@%d,%d) -> %dx%d(%dx%d@%d,%d) "
		"phys 0x%08x -> 0x%08x\n",
		__func__, in->pix.width, in->pix.height,
		in->rect.width, in->rect.height, in->rect.left, in->rect.top,
		out->pix.width, out->pix.height,
	...
	ctx = kcalloc(numtiles, sizeof(*ctx), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
	...
	for (x = htiles - 1; x >= 0; x--) {
		for (y = vtiles - 1; y >= 0; y--) {
			...
			if (y < vtiles - 1) {
				c->in.rect.height++;
				if (out->pix.pixelformat ==
				    V4L2_PIX_FMT_YUV420 ||
				    out->pix.pixelformat ==
				    V4L2_PIX_FMT_YVU420 ||
				    out->pix.pixelformat == V4L2_PIX_FMT_NV12)
					c->in.rect.height =
						round_up(c->in.rect.height, 2);
			}
			...
			if (htiles == 1) {
				...
				if (x) {
					h_resize_coeff = 8192 *
						(dec_width - 1) /
						(out->rect.width - seam_x - 1);
					c->out.rect.left += seam_x;
					c->out.rect.width = out->rect.width -
						c->out.rect.left;
				} else {
					h_resize_coeff = DIV_ROUND_CLOSEST(
						8192 * dec_width, seam_x);
			...
			if (vtiles == 1) {
				...
				if (y) {
					v_resize_coeff = 8192 *
						(dec_height - 1) /
						(out->rect.height - seam_y - 1);
					c->out.rect.top += seam_y;
					c->out.rect.height = out->rect.height -
						c->out.rect.top;

					/* Rounding errors can shift the right
					   pixel over the edge */
					if ((((c->out.rect.height - 1) *
					      v_resize_coeff) + 8191 / 8192) >
					    (c->in.rect.height - 1))
						c->in.rect.height++;
				} else {
					v_resize_coeff = DIV_ROUND_CLOSEST(
						8192 * dec_height, seam_y);
		...
		unsigned long tpm_base, unsigned long vdi_base)
{
	...
	for (i = 0; i < IC_TASK_MAX; i++) {
		...
		task->output_channel = ipu_idmac_get(ipu,
						     templ->output_channel);
